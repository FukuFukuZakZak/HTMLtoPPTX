...
- 「〜の処理ある？」「〜してるコードどこ？」に Serena の `find_symbol` を使わない。意味での検索は better-code-review-graph の `query`（`action=search`）を最初に使う
- Serena はシンボル名が特定できている質問（定義元・参照先・実装クラス・リネーム）にだけ使う
