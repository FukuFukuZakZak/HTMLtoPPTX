...
1. Re-run the visible browser conversion with the user-owned generated-AI-guideline HTML, confirm the new script warning appears while the option is off, then enable it and confirm all 12 sidebar agendas are present in the PPTX.
...
- The paste-mode implementation in `web/index.html`, `web/style.css`, `web/app.js`, and `main_test.go` is currently uncommitted.
- The plain-language script setting, source guidance, dynamic warning, and actual-fixture detector test are currently uncommitted and overlap the paste-mode UI files.
- The conversion-fidelity implementation in `web/app.js`, `web/converter-core.js`, `web/converter-worker.js`, and their JavaScript tests is currently uncommitted and overlaps the existing paste-mode change in `web/app.js`.
