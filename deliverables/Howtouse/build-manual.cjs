...
const chapters = [
  ...
  {nav:'プレビューを確認',group:'基本の操作',title:'ページを切り替えて、細部まで確認する',lead:'最初はページ全体が枠に収まります。右側上部の操作で確認します。',image:'03-preview',marks:[0],caption:'矢印やページ選択で切り替え、＋／−で拡大・縮小。「全体表示」で戻します。',steps:['左右の矢印、またはページ選択で、確認するページを表示します。','＋で拡大し、マウスでつかんでドラッグすると表示位置を動かせます。','−で縮小するか、「全体表示」でページ全体に戻します。'],result:'文字・配置・画像を各ページで確認できたら「PPTXへ変換」に進めます。',note:'プレビューを選択中は、＋／−で倍率、0で全体表示、矢印キーで移動、Page Up／Downでページを操作できます。区切りを検出できないHTMLは文書全体を表示します。スクリプトによる追加内容とPowerPointでの見た目は、変換後のPPTXで確認してください。',extra:'<a class="inline-link" href="#chapter-6">問題がなければ、変換へ進む</a>'},
