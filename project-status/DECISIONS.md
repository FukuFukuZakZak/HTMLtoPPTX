...
- Preview remains static. Use a sandbox with `allow-same-origin` solely for parent measurement and no script permission, explicit restrictive CSP, inert embedded content, and remove scripts/navigation/focus elements from the preview copy. The user's textarea and conversion document are separate and remain untouched by viewer controls. Script-created contents continue through the existing conversion option, with this limit stated in the UI/manual. No dependency or server change is needed for the viewer.
- Most users paste HTML generated by exaBase and use external editors for manual coding. Keep the native textarea and add an explicit `簡易補正` button, visible change/warning feedback and immediate preview. Do not bundle CodeMirror/Monaco or build a custom history stack for this workflow.
- Repair source with a locally bundled HTML5 parser (parse5 8.0.1), preserving all text outside inserted tags or a standalone Markdown wrapper. Automatic additions are limited to structural elements such as div/section/headings/lists. Honor legal optional ends and void tags. Indentation is a hint, so the UI asks users to confirm preview; missing inline formatting boundaries, starts/attributes/content, raw text and foreign-content damage cannot be reliably reconstructed. Warnings leave the whole source unchanged.
- Apply all repairs from one click as one native editing transaction using `execCommand("insertText")`. Do not invoke editing from input/beforeinput/undo/redo handlers. Native Ctrl+Z/Ctrl+Y, no-op repair, branch invalidation, IME guard and navigation are browser acceptance requirements. Keep repair disabled while converting or composing text.
